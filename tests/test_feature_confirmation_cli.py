...
class FeatureConfirmationCliTest(unittest.TestCase):
    def test_valid_human_contract_is_promoted_and_releases_baselines(self) -> None:
        with tempfile.TemporaryDirectory() as directory:
            ...
            confirmed_contract = json.loads(
                (confirmed_dir / "shared_feature_contract.json").read_text()
            )
            manifest = json.loads((confirmed_dir / "manifest.json").read_text())
            self.assertEqual(confirmed_contract["selected_features"], ["area", "perimeter"])
            self.assertEqual(manifest["status"], "confirmed")
            self.assertEqual(
                manifest["decision"]["sha256"],
                hashlib.sha256((confirmed_dir / "shared_feature_contract.json").read_bytes())
                .hexdigest(),
            )
    ...
    def test_confirmation_dry_run_reports_gate_without_writing(self) -> None:
        with tempfile.TemporaryDirectory() as directory:
            ...
            result = main(
                ["confirm-features", "--dry-run", "--config", str(fixture.config_path),
            ...
            self.assertIn("feature_confirmation: released", stdout.getvalue())
            self.assertFalse((fixture.root / "evidence").exists())
    ...
    def test_confirmation_dry_run_explains_a_blocked_gate_without_writing(self) -> None:
        with tempfile.TemporaryDirectory() as directory:
            ...
            self.assertEqual(result, 0, stderr.getvalue())
            self.assertIn("feature_confirmation: blocked", stdout.getvalue())
            self.assertIn("report_sha256 was", stdout.getvalue())
            self.assertFalse((fixture.root / "evidence").exists())
...
def run_confirmation(
    ...
    stdout, stderr = io.StringIO(), io.StringIO()
    result = main(
        ["confirm-features", "--config", str(fixture.config_path),
         "--contract", str(contract_path), "--report", str(report_path)],
        stdout=stdout, stderr=stderr,

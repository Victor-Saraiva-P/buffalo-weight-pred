...
## CNN com fusão tardia de geometria pura

Uma avaliação separada comparou ResNet-18 usando os canais máscara, contorno e
distância com a mesma rede acrescida das dez features permitidas na avaliação
de geometria pura. As duas configurações usaram os mesmos cinco folds e uma
subdivisão interna do treino para early stopping.

| Configuração | Entrada | MAE OOF (kg) | R² OOF |
|---|---|---:|---:|
| ResNet-18 mask-only | máscara/contorno/distância | **54,12** | **0,761** |
| ResNet-18 com fusão tardia | máscara + geometria pura | 56,29 | 0,747 |
| Random Forest geometry-only | geometria pura | 56,10 | 0,708 |

A CNN mask-only reduziu o MAE em 1,98 kg contra o Random Forest geometry-only.
A fusão tardia piorou 2,17 kg contra a CNN equivalente, portanto as features
explícitas não foram incorporadas ao melhor modelo desta rodada. Como a escolha
da arquitetura foi informada por explorações anteriores nos mesmos 132 animais,
o resultado é exploratório e não substitui validação prospectiva externa.

Reprodução: `PYTHON=.venv/bin/python make train-mask-geometry`. Os artefatos
ficam em `generated/mask_geometry/`.
